...
複数のtech-longlist-pipelineエージェントを並列制御し、大規模技術調査リストを高速処理するオーケストレーター。
...
2. **並列実行** - 複数のtech-longlist-pipelineエージェントを起動
3. **進捗監視** - 各エージェントの実行状態を追跡
4. **結果統合** - 完了後、単一Excelファイルに統合
...
**使用例**:
```bash
python merge_results.py work_dir/output
```
...
### 最終成果物
- `技術ロングリスト_最終統合版.xlsx` - 統合された技術ロングリスト
- `統合レポート.md` - 統合統計とサマリー

### 中間ファイル
- `work_dir/input/調査リスト_エージェントN.xlsx` - 分割データ
- `work_dir/output/agent_N/技術ロングリスト_修正版.xlsx` - エージェント別結果
- `work_dir/output/agent_N/verification_report_*.md` - 検証レポート
